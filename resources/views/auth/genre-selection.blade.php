...
<body>
    ...
    <div class="login-container" style="max-width: 520px;">
        <h1 class="logo" style="color: white; font-weight: 800; text-transform: uppercase">Welcome {{ auth()->user()->name }}!</h1>
        <p class="small-text">Select 3 genres that interest you!</p>

        @include('components.alert')

        <form id="genreForm" action="{{ request()->routeIs('preferences.edit') ? route('preferences.store', ['from' => 'edit']) : route('preferences.store') }}" method="POST">
            @csrf
            ...
            <input type="hidden" name="categories" id="selectedCategories" value="{{ json_encode($selectedCategories ?? []) }}">
