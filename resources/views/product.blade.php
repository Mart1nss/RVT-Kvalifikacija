...
@endif
    ...
    <div class="item-container3">
      ...
      <form class="upload-book-form" action="{{url('uploadbook')}}" method="post" enctype="multipart/form-data"> 
        ...
        <p class="max-file-size-text">max file size <span class="highlight-text">10 mb </span>/ format: <span class="highlight-text">pdf</span></p>
      ...
      @foreach ($data as $data)
        <div class="pdf-item" data-genre="{{ $data->category->name ?? '' }}">
          ...
          <div class="button-container" style="display: flex; justify-content: space-between;">
            ...
            <form id="deleteForm{{ $data->id }}" action="{{ route('delete', $data->id) }}" method="POST" style="margin: 0;">
              @csrf
              @method('DELETE')
              <button type="button" class="remove-btn2" onclick="confirmDelete('{{ $data->title }}', '{{ $data->author }}', {{ $data->id }})"><i class='bx bx-trash'></i></button>
            </form>
          </div>
        </div>
    ...
    <div id="deleteModal" class="modal">
        <div class="modal-content">
            ...
            <p id="deleteBookDetails" class="modal-details"></p>
            <div class="modal-buttons">
                <button class="cancel-delete" onclick="closeDeleteModal()">Cancel</button>
                <button class="confirm-delete" onclick="submitDelete()">Yes, Delete</button>
            </div>
        </div>
    </div>
